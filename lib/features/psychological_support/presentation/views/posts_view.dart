...
class PostsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(title: 'منشورات'),
      body: ListView.separated(
        padding: const EdgeInsets.all(16),
        itemCount: posts.length,
        separatorBuilder: (_, __) => const SizedBox(height: 14),
        itemBuilder: (context, index) => PostDetailsCard(posts[index]),
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.pushNamed(context, Routes.createContent, arguments: NewPostConfig.post()),
...
class PostDetailsCard extends StatelessWidget {
  ...
  const PostDetailsCard(this.post, {super.key});
